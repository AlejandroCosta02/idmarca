...
interface StepperProps {
  ...
  onLastStepButtonClick?: () => void;
  onInvalidNext?: (step: number) => void;
}
...
export default function Stepper({
  ...
  return (
    ...
    >
      <div
        className={`mx-auto w-full max-w-md rounded-4xl shadow-xl bg-background border border-border ${stepCircleContainerClassName}`}
      >
        ...
        {!isCompleted && (
          <div className={`px-8 pb-8 ${footerClassName}`}>
            ...
            >
              ...
              <button
                onClick={nextButtonProps.disabled
                  ? () => { if (onInvalidNext) onInvalidNext(currentStep); }
                  : isLastStep
                    ? handleComplete
                    : () => {
                        setDirection(1);
                ...
                {...nextButtonProps}
              >
                {isLastStep ? (lastStepButtonText || "Complete") : nextButtonText}
              </button>
            </div>
          </div>
        )}
